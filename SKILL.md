...
description: Functional TypeScript coding guide. Use when writing, reviewing, or refactoring TypeScript code.
metadata:
  version: '1.0.0'
---
...
- Never use methods for operations that clearly and fully mutate an object. This may be unavoidable in more obscure cases of domain logic, but should be avoided in clearer situations. Instead, use context managers that expose only the methods available at each necessary step and perform the mutative operations between those steps. For example, don't add a `close()` method, since calling it would make the behavior of the other methods predictably fail afterward. Use a context manager instead.
